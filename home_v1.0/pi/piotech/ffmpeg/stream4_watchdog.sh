...
echo Starting stream4 watchdog. Waiting 20 seconds for ffmpeg to initialize...
sleep 20

filename="/home/pi/piotech/config/stream4_watchdog_config.txt"
...
do
     echo !!!!!!!!!!!!
     #timestamp 
     TIMESTAMP_str_1=`ls -l --full-time /tmp/ffmpeg_stream4.log`
     echo $TIMESTAMP_str_1

     #runtime
     RT_str_1=`grep out_time_ms= /tmp/ffmpeg_stream4.log | tail -1`
     echo $RT_str_1
     ...
     #timestamp
     TIMESTAMP_str_2=`ls -l --full-time /tmp/ffmpeg_stream4.log`
     ...
     if [ "$TIMESTAMP_str_1" = "$TIMESTAMP_str_2" ]; then
          ...
          supervisorctl restart stream4 
          sleep 20
     else
          echo Log timestamp changed, check runtime

          #runtime
          RT_str_2=`grep out_time_ms= /tmp/ffmpeg_stream4.log | tail -1`
          echo $RT_str_2
          if [ "$RT_str_1" = "$RT_str_2" ]; then
               ...
               if [ $RT_num -gt ${links[2]} ]; then
                    ...
                    #bitrate
                    BR_str=`grep bitrate= /tmp/ffmpeg_stream4.log | tail -1`
                    echo $BR_str
                    ...
                    if [ $BR_num -lt ${links[0]} ]; then
                         ...
                         #fps
                         FPS_str=`grep fps= /tmp/ffmpeg_stream4.log | tail -1`
                         echo $FPS_str
